...
1.  Run `npm run build`, which will compile all the necessary files to the `public` folder.
2.  Upload the contents of the `public` folder to your web server's root folder.
...
For converting a set of images into a spritesheet add your image to `src/sprite` folder.
...
To convert a bunch of svg files to a single svg sprite add svg file to `src/icons` folder.
Then you can use icon mixin call in your template.

Example:

1.  You copy file icon-name.svg to `src/icons` folder.
...
Build a project and pack content of `public` folder in zip file
...
Create empty module by name in `src/modules` folder
